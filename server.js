...
const dbConnectionString =
  "mongodb+srv://RWUser:8YSSSCTEO4Apnfsm@cluster0.p9g9p.mongodb.net/MarQuiz-DB?retryWrites=true&w=majority";
...
app.get("/quizes", async (req, res) => {
  ...
  try {
    ...
    packs.forEach((p, p_idx) => {
      p.categories.forEach((c, c_idx) => {
...
app.get("/categories/:id", async (req, res) => {
  ...
  try {
    ...
    const returnData = await categoriesCollection.findOne({
...
app.delete("/questions/:id", async (req, res) => {
  ...
  try {
    ...
    const deletedQuestion = await questionsCollection.findOneAndDelete({
...
// For deployment on Strato (serve frontend)
...
io.on("connection", (socket) => {
  ...
  socket.on("buzz", async () => {
    ...
    if (state.buzzer === buzzer.OPEN) {
      ...
      const users = await getConnectedUsers();
      const buzzingUserBySocketId = users.find(
